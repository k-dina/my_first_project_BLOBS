...
# надо как-то обновлять положение блобсов в other_blobs
# может, сделать для этого отдельную функцию?
...
# тут алгоритм: все блобсы идут в ту клетку, которую первую нашли (а лучше
# рандом из ближайшего круга) если никого нет, все просто
# конфликт: смотрим, сколько претендует других блобсов
# и тут можно строить какой-то алгоритм. выводить вероятнсть выигрыша каждого из блобсов
# вынести логику каждую в свой отдельный класс
# есть менеджеры, отвечающие за: поиск еды, реализацию потомства, конфликты
# менеджер - класс, в который передается поле и блобсы. меняет состояние поля
# пример менеджера по уменьшению здоровья каждый час:
# приоритетная очередь в качестве очереди сообщений на инструкции по тикам
# entity-components-system | game ecs approach (посмотреть библиотеки)
